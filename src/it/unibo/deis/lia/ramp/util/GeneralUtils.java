...
//    }
	...
	public static synchronized void refreshMyPublicIpAddress() throws Exception {
		...
		do {
			try {
				System.out.println("Util.myPublicIp index="+index+" urlIpRresolvers.length="+urlIpRresolvers.length+" urlIpRresolvers["+index+"]="+GeneralUtils.urlIpRresolvers[index]);
		...
		if(myIp == null) throw new Exception();
	}
	...
	public static final byte[] unsignedShortToByteArray(int value){
		...
		if(value < 0 || value > 65535) throw new IllegalArgumentException("value must be between 0 and 2^16 - 1 (=65535) included");
	...
	// credits: http://javaswamy.blogspot.com/2004/12/jsse-how-to-ignore-certificateexceptio.html
