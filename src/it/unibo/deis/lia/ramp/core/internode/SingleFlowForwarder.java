 ...
 *
 */
public class SingleFlowForwarder implements DataPlaneForwarder {
	
	private static final int MAX_ATTEMPTS = 8000;
	
	private static SingleFlowForwarder singleFlowForwarder = null;
	...
	public synchronized static SingleFlowForwarder getInstance() {
		if (singleFlowForwarder == null) {
			singleFlowForwarder = new SingleFlowForwarder();
	...
	public void receivedTcpUnicastPacket(UnicastPacket up) {
		...
		if (up.getFlowId() != GenericPacket.UNUSED_FIELD && up.getDestNodeId() != Dispatcher.getLocalRampId()) {
			...
			// If the flow has the highest priority, save the send informations and occupy the transmission channel
