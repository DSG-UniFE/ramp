    ...
    public PathDescriptor selectPath(int sourceNodeId, int destNodeId, ApplicationRequirements applicationRequirements, Map<Integer, PathDescriptor> activePaths) {
        ...
        while (availablePaths.hasNext()) {
            ...
            for (int i = 0; i < path.getEdgeCount(); i++) {
                ...
                System.out.println("FewestIntersectionsFlowPathSelector: next node address " + edge.getAttribute("address_" + nextNode.getId()));
                System.out.println("FewestIntersectionsFlowPathSelector: current node nodeId " + currentNode.getId());
            }
            nodeIds.add(Integer.parseInt(path.getNodePath().get(path.getNodePath().size()-1).getId()));
            System.out.println("FewestIntersectionsFlowPathSelector: current node nodeId " + Integer.parseInt(path.getNodePath().get(path.getNodePath().size()-1).getId()));
